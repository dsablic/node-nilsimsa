...
exports.Nilsimsa = class {
  ...
  static compare (hash1, hash2, encoding='hex') {
    ...
    for (let octet_idx = 0; octet_idx < 32; octet_idx++) {
      const byte1 = buf1[octet_idx];
      const byte2 = buf2[octet_idx];

      for (let byte_offset = 1; byte_offset < 256; byte_offset <<= 1) {

        bit_diff_sum += (byte1 & byte_offset) === (byte2 & byte_offset) ? 0 : 1;
      }
    }
