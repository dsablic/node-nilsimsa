...
exports.Nilsimsa = class {
  ...
  static compare (hash1, hash2, encoding='hex') {
    ...
    for (let byte_idx = 0; byte_idx < 32; byte_idx++) {
      const byte1 = buf1[byte_idx];
      const byte2 = buf2[byte_idx];

      for (let bit_offset = 1; bit_offset < 256; bit_offset <<= 1) {
        bit_diff_sum += (byte1 & bit_offset) === (byte2 & bit_offset) ? 0 : 1;
      }
    }
