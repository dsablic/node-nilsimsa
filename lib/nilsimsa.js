...
function decodeHash(str, encoding) {
  if (encoding === 'base64') return base64ToBytes(str)
...
export class Nilsimsa {
  ...
  static compare(hash1, hash2, encoding = 'hex') {
    const buf1 = decodeHash(hash1, encoding)
    const buf2 = decodeHash(hash2, encoding)
  ...
  digest(encoding = 'hex') {
    ...
    code.reverse()
    return encodeBytes(code, encoding)
  }
}
